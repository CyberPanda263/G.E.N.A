...
Платформа працює за принципом **"Усе своє ношу з собою"**. Вам більше не потрібні зовнішні CI-runner'и чи хмарні S3-бакети.
1. **CI/CD Bridge:** Ви робите `git push`, вебхук смикає Tekton EventListener.
2. **Build:** Tekton (через Kaniko) збирає Docker-образ прямо всередині кластера і пушить його у локальний реєстр (`registry.registry.svc.cluster.local:5000`).
3. **Deploy:** ArgoCD помічає зміну маніфестів і розгортає оновлений образ.
4. **Backup:** Velero автоматично зберігає конфігурацію кластера у нативний in-cluster MinIO бакет.
...
## ⚙️ Що ви можете розгорнути на G.E.N.A
